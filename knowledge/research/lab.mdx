...
The first Nox30 drive sweep against `TopBoost-Gain5` found the lowest
log-spectral distance around `drive = 0.800`. Treat that as a coarse anchor, not
as a final calibration: the null residual and envelope metrics still indicate
dynamic mismatch that needs a broader sweep or targeted greybox work.
