...
use greybound::{
    amp_model_descriptor, configure_minotaur_clip_neural, configure_minotaur_tone_neural,
    BrigadeControls, CelesteControls, ControlDescriptor, ControlKind, DartfordControls,
...
fn parse_neural_cell_override(value: &str) -> Result<NeuralCellOverride> {
    let (component, descriptor) = value
        .split_once('=')
        .context("--neural-cell expects COMPONENT=DESCRIPTOR, for example nox30.first_stage=lab/models/common-cathode-12ax7-mlp-v1/model.greybound.json or minotaur.tone=lab/models/klon-tone-mlp-current/model.greybound.json")?;
...
fn apply_neural_overrides(
    ...
    for override_ in overrides {
        match override_.component.as_str() {
            ...
            other => bail!(
                "unsupported --neural-cell component '{}'; supported: nox30.first_stage, minotaur.clip, minotaur.tone",
                other
            ),
        }
    }
...
fn print_help() {
    eprintln!(
        "Usage: greybound-cli --rig PATH|- [OPTIONS]\n\
         ...
         \x20 --neural-cell COMPONENT=PATH  Run a neural counterpart for a supported component: nox30.first_stage, minotaur.clip, minotaur.tone\n\
